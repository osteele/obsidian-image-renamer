import { App, Editor, MarkdownView, Modal, Notice, Plugin, PluginSettingTab, Setting, TFile, normalizePath, requestUrl } from 'obsidian';
...
export default class ImageRenamerPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			name: 'Rename image file with AI caption',
		...
		this.addCommand({
			...
			name: 'Rename image file interactively',
			checkCallback: (checking: boolean) => {
				const file = this.app.workspace.getActiveFile();
				if (file && this.isImageFile(file)) {
					if (!checking) {
		...
		this.registerEvent(
			this.app.workspace.on('file-menu', (menu, file) => {
				if (file instanceof TFile && this.isImageFile(file)) {
					menu.addItem((item) => {
						item
							.setTitle('Rename with AI caption')
							.setIcon('image')
							.onClick(() => {
								this.renameImageFile(file);
							});
		...
		this.addSettingTab(new ImageRenamerSettingTab(this.app, this));
	}
	...
	async generateCaptionsWithRetry(file: TFile, retryCount: number = 0): Promise<string[]> {
		try {
	...
	async generateCaptions(file: TFile): Promise<string[]> {
		...
		if (!this.validateApiEndpoint(this.settings.apiEndpoint)) {
		...
		const base64 = await this.resizeAndConvertToBase64(arrayBuffer, file.extension);
		
		// Create abort controller for timeout
		this.abortController = new AbortController();
		const timeoutId = setTimeout(() => this.abortController?.abort(), this.settings.timeoutMs);
		...
		try {
					...
					messages: [
						{
							...
							content: [
								{
									type: 'text',
									text: 'Generate 3 different concise captions for this image. Each caption should be suitable as a filename (2-5 words). Return them as a JSON array of strings.'
								},
								{
									type: 'image_url',
									image_url: {
			...
			if (!response.ok) {
				...
				if (response.status === 401) {
					...
					errorMessage = 'Bad request. Please check your model name and settings.';
			...
			try {
				...
				if (Array.isArray(captions)) {
					return captions.map(c => this.sanitizeFilename(c));
				}
			} catch (e) {
				// Fallback: try to extract array from text
				const match = content.match(/\[([^\]]+)\]/);
				if (match) {
					return match[1].split(',').map((s: string) => this.sanitizeFilename(s.trim().replace(/['"]/g, '')));
				}
			}
			
			// Final fallback: use the content as a single caption
			return [this.sanitizeFilename(content)];
		} catch (error) {
			clearTimeout(timeoutId);
	...
	async resizeAndConvertToBase64(arrayBuffer: ArrayBuffer, extension: string): Promise<string> {
	...
	async filterDuplicateNames(captions: string[], file: TFile): Promise<string[]> {
		...
		for (const caption of captions) {
			...
			const testPath = normalizePath(`${parentPath}/${finalName}.${file.extension}`);
	...
	async renameImageFile(file: TFile) {
		const progressNotice = new Notice('Generating caption...', 0);
		...
		try {
			...
			if (availableCaptions.length === 0) {
				...
				if (newAvailable.length === 0) {
					new Notice('Could not generate unique names. Please try again or use interactive mode.');
					return;
				}
	...
	async renameImageFileInteractive(file: TFile) {
		const progressNotice = new Notice('Generating captions...', 0);
		...
		try {
			...
			if (availableCaptions.length === 0) {
				new Notice('All generated names already exist. Generating new options...');
				const newCaptions = await this.generateCaptionsWithRetry(file);
				const newAvailable = await this.filterDuplicateNames(newCaptions, file);
				
				if (newAvailable.length === 0) {
					new Notice('Could not generate unique names. Please enter a custom name.');
					// Still show modal with empty suggestions so user can enter custom name
					new CaptionSelectionModal(this.app, this, file, [], async (caption) => {
						await this.performRename(file, caption);
					}).open();
				} else {
					new CaptionSelectionModal(this.app, this, file, newAvailable, async (caption) => {
						await this.performRename(file, caption);
					}).open();
				}
			} else {
				new CaptionSelectionModal(this.app, this, file, availableCaptions, async (caption) => {
					await this.performRename(file, caption);
				}).open();
			}
		} catch (error) {
	...
	async performRename(file: TFile, newName: string) {
		...
		const newPath = normalizePath(`${file.parent?.path || ''}/${finalName}.${file.extension}`);
		
		// Check for duplicate
		const existingFile = this.app.vault.getAbstractFileByPath(newPath);
		if (existingFile && existingFile !== file) {
			new Notice(`A file named "${finalName}.${file.extension}" already exists`);
			return;
		}
...
class CaptionSelectionModal extends Modal {
	...
	constructor(app: App, plugin: ImageRenamerPlugin, file: TFile, captions: string[], onSubmit: (caption: string) => void) {
		super(app);
		this.plugin = plugin;
		this.file = file;
	...
	async onOpen() {
		...
		if (this.captions.length > 0) {
			const captionContainer = contentEl.createEl('div', { cls: 'caption-selection-container' });
		...
		textInput.addEventListener('input', async () => {
			...
			if (value) {
				...
				const testPath = normalizePath(`${this.file.parent?.path || ''}/${testName}.${this.file.extension}`);
				const existingFile = this.app.vault.getAbstractFileByPath(testPath);
				
				if (existingFile && existingFile !== this.file) {
					textInput.style.borderColor = 'var(--text-error)';
					if (!textInput.nextElementSibling || !textInput.nextElementSibling.classList.contains('duplicate-warning')) {
					...
					const warning = textInput.parentElement?.querySelector('.duplicate-warning');
					warning?.remove();
				}
			}
		});
...
class ImageRenamerSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const warningDiv = containerEl.createEl('div', {
			cls: 'setting-item-description',
			attr: {
				style: 'background: var(--background-modifier-error); padding: 12px; border-radius: 5px; margin-bottom: 20px; color: var(--text-normal);'
			}
		});
		warningDiv.createEl('strong', { text: '⚠️ Security Note: ' });
		warningDiv.createEl('span', { 
			text: 'Your API key is stored in plain text in your vault\'s configuration. Keep your vault secure and avoid sharing it with untrusted parties.'
		});
		...
		const apiKeySetting = new Setting(containerEl)
			...
			.addText(text => {
				...
				const eyeIcon = text.inputEl.parentElement?.createEl('span', {
					cls: 'clickable-icon',
					attr: {
						style: 'position: absolute; right: 60px; top: 50%; transform: translateY(-50%); cursor: pointer; z-index: 1;' // Moved left to avoid border overlap
					}
				});
					...
					try {
						...
						if (response.ok) {
							...
							if (data.data && Array.isArray(data.data)) {
								...
								const hasVisionModel = data.data.some((m: any) => 
									m.id.includes('vision') || m.id.includes('gpt-4') || m.id.includes('claude')
								);
								new Notice(`✅ API connected! Found ${modelCount} models${hasVisionModel ? ' (including vision models)' : ''}`);
							} else {
